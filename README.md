...
- [x] Mock AI 分类预填（「AI 已识别」标识 + 建议名称，低置信度提示手选）
- [x] 瀑布流分类筛选 Tab（含「组合」视图、选择持久化）、顶部随滚动方向显隐
- [x] 创建组合（多选 2-8 件 → 场景标签/自定义 → 拼图封面 → 混排展示 → 组合详情）
- [x] 存储管理（图片质量选项、清理未引用图片）、Mock 订阅页（权益对比 + 意愿埋点）、昵称编辑
- [x] 缩略图缓存（内存 + 磁盘双层）、新卡片入场动画与回顶定位
- [ ] 账号登录（Sign in with Apple，需本机配置签名权限）
